...
import com.quackware.shake.wiigee.logic.*;
import com.quackware.shake.wiigee.event.*;
import com.quackware.shake.wiigee.filter.*;
...
public class Device {
    ...
    protected Vector<ActionListener> mActionListeners = new Vector<ActionListener>();
    ...
    public Device(boolean autofiltering) {
        ...
        this.addActionListener(this.processingunit);
    }
    ...
    public void fireActionStartEvent(int actionType)
    {
    	ActionEvent ae = new ActionEvent(this,actionType,ActionEvent.START_TYPE);
    	for(int i = 0;i<this.mActionListeners.size();i++)
    	{
    		this.mActionListeners.get(i).actionStartReceived(ae);
    	}
    	if(ae.isRecognitionEvent() || ae.isTrainEvent())
    	{
    		this.resetAccelerationFilters();
    	}
    }
    
    public void fireActionStopEvent(int actionType)
    {
    	ActionEvent ae = new ActionEvent(this,actionType,ActionEvent.STOP_TYPE);
    	for(int i = 0;i<this.mActionListeners.size();i++)
    	{
    		this.mActionListeners.get(i).actionStopReceived(ae);
    	}
    }
